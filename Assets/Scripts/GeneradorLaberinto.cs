...
public class GeneradorLaberinto : MonoBehaviour
{
    ...
    public Color colorPared = Color.black;
    ...
    private int[,] _grid; 
    private HashSet<Vector2Int> _caminoSolucion = new HashSet<Vector2Int>();
    ...
    public void Generar()
    {
        ...
        foreach (Transform child in contenedor) {
            if (child.gameObject.name != "StartPoint" && child.gameObject.name != "GoalPoint")
    ...
    void CalcularSolucion(Vector2Int inicio, Vector2Int meta)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            if (current == meta) {
                _caminoSolucion = new HashSet<Vector2Int>(path);
                return;
            }
    ...
    void ConfigurarMarcadores(Vector2Int inicio, Vector2Int meta)
    {
        ...
        // Auto-crear marcadores si son nulos
        if (puntoInicio == null) {
        ...
        if (puntoMeta == null) {
            GameObject go = new GameObject("GoalPoint");
            go.transform.SetParent(contenedor.parent, false);
        ...
        if (puntoInicio != null) {
            ...
            puntoInicio.GetComponent<Image>().color = Color.white;
            CrearTexto(puntoInicio.gameObject, "INICIO", Color.black);
        ...
        if (puntoMeta != null) {
            ...
            puntoMeta.GetComponent<Image>().color = Color.white;
            puntoMeta.gameObject.name = "GoalPoint"; // Asegurar nombre para colisión
            CrearTexto(puntoMeta.gameObject, "META", Color.red);
