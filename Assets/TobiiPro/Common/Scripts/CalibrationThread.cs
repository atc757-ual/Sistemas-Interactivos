...
namespace Tobii.Research.Unity
{
    ...
    public sealed class CalibrationThread
    {
        ...
        public sealed class MethodResult
        {
            ...
            private CalibrationResult _lastResult; // NEW: Para diagnósticos
            ...
            public CalibrationResult LastResult
            {
                get
                {
                    lock (Lock)
            ...
            public void Finished(CalibrationStatus status, int elapsed, CalibrationResult result = null)
            {
                lock (Lock)
                {
                    _ready = true;
                ...
                else
                {
                    switch (currentResult.Command)
                    {
                        ...
                        case MethodResult.CommandType.Compute:
                            CalibrationResult res = null;
                            CalibrationStatus status = CalibrationStatus.Failure;
                            
                            if (screenBasedCalibration != null) {
                                res = screenBasedCalibration.ComputeAndApply();
                                status = res.Status;
                            } else {
                                status = hmdBasedCalibration.ComputeAndApply().Status;
                            }

                            stopWatch.Stop();
                            currentResult.Finished(status, (int)stopWatch.ElapsedMilliseconds, res);
                            break;
